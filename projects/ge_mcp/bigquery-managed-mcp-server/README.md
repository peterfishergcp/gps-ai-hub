...
## 📋 Prerequisites & Organization Policy Setup

### 1. Enable Organization Policy API
Before modifying org policies, ensure the Organization Policy API is enabled:
```bash
gcloud services enable orgpolicy.googleapis.com --project=<PROJECT_ID>
```

### 2. Override Organization Policy Constraint (Custom MCP Data Stores)
By default, Gemini Enterprise blocks the creation of Custom Model Context Protocol (MCP) data stores using a managed organization policy constraint. To enable Custom MCP data stores:
