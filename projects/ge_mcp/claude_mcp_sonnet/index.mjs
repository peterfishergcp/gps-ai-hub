...
/**
 * Call Anthropic Claude Sonnet on Vertex AI (rawPredict / streamRawPredict)
 */
async function callClaudeVertex({
  ...
  const responseText = data.content?.[0]?.text || JSON.stringify(data);
  return responseText;
}

/**
 * A2UI JSON Component Templates for A2A and ADK Integration
 */
const A2UI_EXAMPLES = {
  card: {
    type: "a2ui.Card",
    properties: {
      title: "Sample A2UI Card Component",
      subtitle: "Rendered via A2A Protocol / ADK",
      elevation: 2,
    },
    children: [
      {
        type: "a2ui.Text",
        properties: {
          content: "This is a dynamic A2UI component generated by Claude Sonnet via MCP.",
          variant: "body1",
        },
      },
      {
