...
/**
 * Resolves Google SDP policy / label header or string into Microsoft Purview Sensitivity Label GUID
 */
function translateSdpToPurviewLabel(sdpLabelOrPolicy) {
    if (!sdpLabelOrPolicy) {
        return {
            sdpSource: SDP_PURVIEW_CONFIG.contentPolicy,
            purviewGuid: SDP_PURVIEW_CONFIG.defaultPurviewGuid,
            displayName: "CUSTOM_CEPF_LABEL (Default SDP Mapping)"
        };
    }

    const matched = SDP_PURVIEW_CONFIG.labelMappings[sdpLabelOrPolicy] || 
                    SDP_PURVIEW_CONFIG.labelMappings[sdpLabelOrPolicy.toLowerCase()];

    if (matched) {
        return {
            sdpSource: sdpLabelOrPolicy,
            purviewGuid: matched.purviewGuid,
    ...
    return {
        sdpSource: sdpLabelOrPolicy,
        purviewGuid: SDP_PURVIEW_CONFIG.defaultPurviewGuid,
        displayName: `CUSTOM_CEPF_LABEL (Mapped from ${sdpLabelOrPolicy})`
    };
}

/**
 * Extracts SDP headers from HTTP request
 */
function extractSdpContext(req) {
    const headers = req.headers || {};
    const sdpHeader = headers['x-goog-sdp-policy'] || 
                      headers['x-goog-sdp-label'] || 
                      headers['x-sdp-label'] || 
                      headers['x-sdp-policy'] || 
                      process.env.SDP_DEFAULT_LABEL || 
                      "CUSTOM_CEPF_LABEL";

    return translateSdpToPurviewLabel(sdpHeader);
}
   ...
   *Note: Microsoft SharePoint Graph API search results may be paginated or partial. If you suspect missing results, please try again with a more specific query or document title.*`
            ...
            mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
                ...
                try {
                    ...
                    const sdpContext = extractSdpContext(req);
                    console.error(`[SDP LOG] Policy Context: ${sdpContext.sdpSource} -> Purview Label GUID: ${sdpContext.purviewGuid}`);
                        ...
                        return {
                            content: [{
                                ...
                                text: JSON.stringify({
                                    ...
                                    message: `Access to tool '${name}' is blocked under Google SDP Content Policy '${sdpContext.sdpSource}' (Microsoft Purview Sensitivity Label GUID: ${sdpContext.purviewGuid}).`,
                                    securityMetadata: {
                                        sdpContentPolicy: SDP_PURVIEW_CONFIG.contentPolicy,
                                        sdpPolicySource: sdpContext.sdpSource,
                                        microsoftPurviewSensitivityLabelGuid: sdpContext.purviewGuid,
                                        labelDisplayName: sdpContext.displayName,
                    ...
                    // Attach SDP & Microsoft Purview Sensitivity Label Metadata
                    resultObj.securityMetadata = {
                        sdpContentPolicy: SDP_PURVIEW_CONFIG.contentPolicy,
                        sdpPolicySource: sdpContext.sdpSource,
                        microsoftPurviewSensitivityLabelGuid: sdpContext.purviewGuid,
                        labelDisplayName: sdpContext.displayName
